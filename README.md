...
# --- PROMPT D'EXTRACTION ---
system_prompt = """
Tu es un robot d'extraction. Pour chaque adresse lue, génère UNIQUEMENT ce bloc HTML.
...
# --- LE MOULE HTML SANS JAVASCRIPT (PUR CSS) ---
...
<head>
    ...
    <style>
        ...
        /* BOUTON VUE COMPACTE VIA CHECKBOX (SANS JS) */
        ...
        #compact-toggle:checked ~ .list .v-btn {{ display: none; }}
        #compact-toggle:checked ~ .list .f-btn {{ display: none; }}
        ...
        .card {{ background: white; border-radius: 15px; padding: 20px; display: flex; align-items: center; justify-content: space-between; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border-left: 8px solid #1a73e8; cursor: pointer; position: relative; transition: 0.2s; }}
        ...
        .cb {{ display: none; }}
        .cb:checked + .card {{ background: #f1f5f9; border-left-color: #22c55e; opacity: 0.7; transform: scale(0.96); }}
...
<body>
    ...
    <div class="top-bar">
        <button class="btn-r" onclick="window.location.reload()">🔄 TOUT DÉCOCHER</button>
